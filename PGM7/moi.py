...
def binarize(data,n_c):
    binary_data = np.zeros([data.shape[0],data.shape[1],n_c])
    for c in range(n_c):
...
class MoI:
    def __init__(self,N=10,M=5,n_c=2,nature='Potts', gauge='zerosum',random_state=None):
        ...
        self.gauge = gauge
        self.nature = nature
        self.muh = np.ones(M)/float(M)
        self.cum_muh = np.cumsum(self.muh)
        self.gh = np.zeros(M)
        if nature == 'Potts':
            self.weights = np.zeros([M,N,n_c])
        else:
            self.weights = np.zeros([M,N])
        if nature == 'Bernoulli':
            self.cond_muv =np.ones([M,N])/float(2)
        elif nature == 'Spin':
            self.cond_muv =np.zeros([M,N])
        elif nature == 'Potts':
            self.cond_muv =np.ones([M,N,n_c])/float(n_c)
    ...
    def likelihood(self,data):
        ...
        if self.nature in ['Bernoulli','Spin']:
            ...
            f = cy_utilities.compute_output_C(self.N, self.n_c,data,self.weights)
    ...
    def fit_online(self,data,weights=None,batch_size = 100, learning_rate = 0.01,lr_final=None,n_iter = 10,lr_decay=True,decay_after = 0.5, verbose=1,shuffle_data=True,print_every=5,init_bias = 0.001,init=None):
        ...
        if not init == 'previous':
            ...
            if self.nature == 'Bernoulli':
                self.muvh = np.ones([self.M,self.N])/(2.0*self.M)
            elif self.nature == 'Spin':
                self.muvh = np.zeros([self.M,self.N])
            else:
                self.muvh = np.ones([self.M,self.N,self.n_c])/float(self.n_c * self.M)
        else:
    ...
    def partial_EM(self,data,cond_muh_ijk, indices, weights=None,eps=1e-4,maxiter=10,verbose=0):
        ...
        while not converged:
            if self.nature in ['Bernoulli','Spin']:
                ...
                f = cy_utilities.compute_output_C(self.N, self.n_c,data,self.weights[ [i,j,k],:,:])
