...
if double_precision:
    signature = "(float64[:])(int64,int64,int64[:,:],float64[:,:])"
else:
    signature = "(float32[:])(int64,int64,int16[:,:],float32[:,:])"


@njit(signature,parallel=True,cache=True,nogil=False)
def dot_Potts_C(N,q,config,fields):
    B = config.shape[0]
...
@njit(signature,parallel=True,cache=True,nogil=False)
def dot_Potts2_C(N,q,config,fields):
    B = config.shape[0]
    out = np.zeros(B,dtype=curr_float)
    for b in prange(B):
        for n in prange(N):
...
@njit(signature,parallel=True,cache=True,nogil=False)
def compute_output_C(N,q,config,W):
    B = config.shape[0]
    # N = config.shape[1]
    M = W.shape[0]
    # q = W.shape[2]
...
@njit(signature,parallel=True,cache=True,nogil=False)
def compute_output_C2(M,q,config,W):
    B = config.shape[0]
    # M = config.shape[1]
    N = W.shape[1]
    # q = W.shape[2]
...
if double_precision:
    ...
    signature = "(float32[:,:,:])(int64,int64,int64,int16[:,:],float32[:,:,:,:])"



@njit(signature,parallel=True,cache=True,nogil=False)
def compute_output_Potts_C(N,q_down,q_up,config,W):
    B = config.shape[0]
    M = W.shape[0]
    # N = W.shape[1]
    # q_up = W.shape[2]
    ...
    for m in prange(M):
        for n in prange(N):
            for q_up_ in prange(q_up):
                for b in prange(B):
                    out[q_up_,m,b] += W[m,n,q_up_,config[b,m]]
...
@njit(signature,parallel=True,cache=True,nogil=False)
def compute_output_Potts2_C(M,q_up,q_down,config,W):
    B = config.shape[0]
    # M = W.shape[0]
    N = W.shape[1]
    # q_up = W.shape[2]
...
def log_erf_times_gauss_numba1(x):
    ...
    for i in prange(x.shape[0]):
        out[i] = np.log(erfcx_fn(x[i]/sqrt2_double)) + logsqrtpiover2_double
    return out
...
def get_cross_derivatives_dReLU_numba(V,I,
    ...
    for m in prange(M):
        ...
        dI_plus_ddelta = 1/np.sqrt(gamma_ * (1+eta_) )
        dI_minus_ddelta = 1/np.sqrt(gamma_* (1-eta_) )
        ...
        for b in prange(B):
            I_ = I[b,m]
            weights_ = weights[b]
            I_plus = ( -np.sqrt(1+eta_) * (I_-theta_) + delta_/np.sqrt(1+eta_) )/np.sqrt(gamma_)
            I_minus = ( np.sqrt(1-eta_) * (I_-theta_) + delta_/np.sqrt(1-eta_) )/np.sqrt(gamma_)
            etg_plus = erf_times_gauss_numba0(I_plus)
            etg_minus = erf_times_gauss_numba0(I_minus)
            ...
            e = (I_-theta_) * (1+eta_ * (p_plus-p_minus ) ) - delta_ * (p_plus-p_minus) + 2*eta_*np.sqrt(gamma_)/Z
            v = eta_ * (p_plus-p_minus) + p_plus * p_minus * (2*delta_/np.sqrt(gamma_)- 2 * eta_ * (I_-theta_)/np.sqrt(gamma_)) * (2*delta_/np.sqrt(gamma_)- 2 * eta_ * (I_-theta_)/np.sqrt(gamma_) - np.sqrt(1+eta_)/etg_plus - np.sqrt(1-eta_)/etg_minus)- 2 * eta_*e/(np.sqrt(gamma_)*Z)
            ...
            dI_plus_deta = -1/(2*np.sqrt(gamma_*(1+eta_))) * ( (I_-theta_) + delta_/(1+eta_) )
            dI_minus_deta = -1/(2*np.sqrt(gamma_*(1-eta_))) * ( (I_-theta_) - delta_/(1-eta_) )
            ...
            de_dI = (1+v)
            de_dtheta = -de_dI
            de_dgamma = 2*((I_-theta_) * eta_ - delta_) * dp_plus_dgamma + eta_/(Z*np.sqrt(gamma_)) - 2*eta_*np.sqrt(gamma_)/Z * dlogZ_dgamma
            de_ddelta = -(p_plus-p_minus)  + 2*((I_-theta_) * eta_ - delta_) * dp_plus_ddelta - 2*eta_*np.sqrt(gamma_)/Z * dlogZ_ddelta
            de_deta = (I_-theta_) * (p_plus-p_minus) + 2*( (I_-theta_)*eta_-delta_ ) * dp_plus_deta + 2*np.sqrt(gamma_)/Z - 2*eta_*np.sqrt(gamma_)/Z * dlogZ_deta
