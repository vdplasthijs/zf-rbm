...
def log_erf_times_gauss(x):
    return cy_utilities.log_erf_times_gauss_numba1(x.flatten().astype(np.float64)).reshape(x.shape).astype(curr_float)
...
def bilinear_form(W, X1,X2,c1=1,c2=1):
    ...
    if (c1==1) & (c2==1):
        ...
        out = np.sum(X1 * cy_utilities.compute_output_C(X2.shape[1],c2,X2,W) ,-1)
    elif (c1>1) & (c2 ==1):
        out = cy_utilities.dot_Potts2_C(X1.shape[1],c1,X1, np.tensordot(X2.astype(curr_float),W,(1,1) ) )
    elif (c1>1) & (c2>1):
        out = cy_utilities.dot_Potts2_C(X1.shape[1], c1, X1, cy_utilities.compute_output_Potts_C(X2.shape[1],c2,c1,X2,W) )
